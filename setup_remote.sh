...
REMOTE_HOST=$1
REMOTE_GIT_USER="cberg18"
REMOTE_GIT_URL="github.com"
REMOTE_GIT_SSH_URL="git@$REMOTE_GIT_URL"
REMOTE_GIT_HTTPS_URL="https://$REMOTE_GIT_URL"
...
if ssh "$REMOTE_HOST" "ssh -T -o StrictHostKeyChecking=no $REMOTE_GIT_SSH_URL"; then
    ...
    ssh "$REMOTE_HOST" "echo 'Host $REMOTE_GIT_URL
  IdentityFile ~/.ssh/id_rsa
  User git' >> ~/.ssh/config && chmod 600 ~/.ssh/config"

    # Configure git to use SSH instead of HTTPS for this domain
    ssh "$REMOTE_HOST" "git config --global url.\"$REMOTE_GIT_SSH_URL:\".insteadOf \"$REMOTE_GIT_HTTPS_URL/\""
fi
...
ssh "$REMOTE_HOST" "if [ ! -d ~/.dotfiles/.git ]; then
    ...
    rm -rf ~/.dotfiles
    git clone $REMOTE_GIT_SSH_URL/$REMOTE_GIT_USER/.dotfiles ~/.dotfiles
else
